...
1. Setting up of standard field observation, measurement and/or experimental protocols that work digitally e.g. with KoboToolbox.
...
The associated activities have to be carried out in sequence; but, the actual procedures can certainly be changed over time as different survey or monitoring priorities emerge. This chapter covers the 6 needed steps, including example data, R-code and the relevant links to external resources (where needed).
...
The following provides an example that is based on the need for sampling representative cropland areas in all of Tanzania. The ROI calculations for this example are shown in the chunk below, which encompasses the national area of Tanzania in which cropland is predicted to occur (in 2020). The set up, packages and data needed for this exercise are also found below. Note that cropland occurrence is based on comprehensive GeoSurveys, which were conducted in 2017-2018 (see chapter ...). You'll need the following R-packages:
