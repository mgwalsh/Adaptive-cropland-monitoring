...
*..., who lost their lives during a tragic incident while on field survey in ... are gratefully acknowledged for both their contributions and their sacrifice ...*

## Introduction {#FS-intro}

This chapter illustrates procedures for organizing and conducting field surveys as well as physical (soil, plant and livestock) sample collections and their curations. This is the practical, field-based side-of-things, which get field observations and physical (rock, soil, plant and potentially livestock biomarker) samples assembled and databased in an efficient and consistent manner. Because field survey activities are expensive, but also because of their particulary high value to any subsequent analyses, it is very important to plan and execute these activities carefully. We can think of field surveys, monitoring and experiments as occurring in 5-steps.

1. Delineation of a region of interest.
2. Drawing a spatially representative sample of locations from within the ROI, considering field crew safety factors first-and-foremost.
3. Setting up of standard field observation and measurement schema that work digitally.
...
Next is the sample frame setup and a definition of the ROI, which can be changed as needed. Note that apart from the cropland area mask, we have also included a *"distance to major road"* covariate. From a practical perspective this is included because of the typically extreme logistics associated with collecting field samples in Tanzania. However, one can always change the `rdt` (distance to road) variable in the script chunk to accommodate any other specific coverage reuirements. For example, one could also use additional constraint variables where needed.    
...
This chunk then performs the steps of drawing
