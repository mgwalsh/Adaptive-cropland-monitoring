...
This chapter illustrates procedures for organizing and conducting field surveys as well as for collecting physical (soil, plant and livestock) samples. This is the practical, field-based side-of-things, which provides field observations and physical (rock, soil, plant and/or potentially livestock biomarker) samples that are assembled and databased in a cost efficient and consistent manner. 

Field survey activities are fairly expensive to implement, but also but also have particulary high value for any subsequent analyses. So, it is very important to plan and execute the associated activities carefully. We can think of field surveys, monitoring and/or experiments as occurring in 6 sequential steps:

1. Delineation of a region of interest (ROI).
...
1. Finally, repeating and updating these steps over time.

The associated activities have to be carried out in sequence; but, the actual procedures can certainly be changed as different survey or monitoring priorities emerge. This chapter covers the 6 needed steps, including example data, R-code and the relevant links to external resources (where needed).
