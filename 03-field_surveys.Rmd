...
4. Conducting the field sampling and collecting the needed georeferenced physical samples or monitoring data.
5. Sample processing and curation.

The associated activities essentially have to be carried out in sequence, but the actual procedures can be adjusted or changed over time as different monitoring priorities emerge. This chapter covers those 5 needed steps, including example data, R-code and the relevant links to external resources.
...
This provides a plot of the ROI, and extracts the needed spatial coordinates of the intended sampling frame. The ROI can be plotted as an R raster object.

```{r}
plot(roi, axes=F, legend=F)
```

## Drawing a sampling plan {#samp_plan}
...
It can also be exported as a GPS readable file
