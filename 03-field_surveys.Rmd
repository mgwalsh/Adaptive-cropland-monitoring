---
title: "Field surveys, tags and sample collections"
author: M. G. Walsh
date: "21/11/2020"
output:
  html_document
...
This notebook illustrates AfSIS procedures and methods for conducting field surveys and for collecting physical samples. This is the practical, field-based side-of-things, which provides survey designs and procedures for physical measurements of any (physical) rock, water, soil, plant and/or livestock fecal or blood) samples, which can be georeferenced, measured and databased in a consistent manner.

Field survey activities are expensive to implement, but they also provide particulary high information value for any subsequent analyses and cropland management decisions. So, it is very important to plan and execute the associated field activities carefully. We can think of field surveys, observations, monitoring and experiments as occurring in 6 interdependent steps:
...
The associated activities should be carried out in sequence; but, the actual procedures can certainly be changed as different survey or monitoring priorities emerge over time. This chapter covers the 6 steps including example data, R-code and the relevant links to external open-source resources.
...
The following provides an example that is based on the need for physically sampling representative cropland areas of all of Rwanda. The ROI calculations for this example are shown in the chunk below. Note that cropland occurrences are based on comprehensive GeoSurveys, which were conducted in 2019 & 2020. The set up, packages and data needed for this exercise are also found below. You'll need the following R-packages to run the example:
...
The data for running this Rwanda example are accessible via the following chunk:

```{r}
# download current Rwanda GeoSurvey predictions
...
Next is the sample frame setup and a working definition of the region of interest (ROI). Note that apart from the (GeoSurvey) cropland area mask for Rwanda, we have also included a "distance to buildings" covariate. By neccessity, cropping and other land management activities are closely associated with the presence of buildings and humans. From a practical perspective this is included because of the typically extreme logistics and potential hazards to field crews associated with collecting samples in remote, unpopulated locations. However, one can always change the `bd` (distance to buildings) variable in the script chunk directly below to accommodate any other specific sampling frame coverage requirements. One could also use additional constraint variables such as steep slopes or other accessibility constraints where needed.
...
# save widget
saveWidget(w, 'RW_sample.html', selfcontained = T)
```
...
1. Sticky QR code sample labels, 2 for each soil sample container.
