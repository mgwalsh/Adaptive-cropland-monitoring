...
This chapter illustrates procedures for organizing and conducting field surveys as well as physical (soil, plant and livestock) sample collections and their physical curation. This is the practical, field-based side-of-things, which get field observations and physical (rock, soil, plant and potentially livestock biomarker) samples assembled and databased in an efficient and consistent manner. Field survey activities are expensive, but also but also have particulary high value to any subsequent analyses. So, it is very important to plan and execute these activities carefully. We can think of field surveys, monitoring and/or experiments as occurring in 5 sequential steps:
...
The following provides an example that is based on the need for sampling representative cropland areas in all of Tanzania. The ROI calculations for this example are shown in the chunk below, and encompasses the national area of Tanzania in which croplands are predicted to occur (in 2020). The set up, packages and data needed for this exercise are also found below. Note that cropland occurrence is based on comprehensive GeoSurveys, which were conducted in 2017-2018 (see chapter ...).
...
This provides a plot of the ROI, and extracts the needed spatial coordinates of the intended sampling frame.
...
The following chunk then performs the steps of drawing a spatially (geographically) balanced sample from the specified ROI. We have used the `lcube` package from the `BalancedSampling` library in R (R Core Team, 2017; Grafström and Lisic, 2016) to draw the sample here. This implements the cube method of Deville and Tillé (2004), which allows one to sample based on inclusion probabilities while aiming for balance and spread with respect to speciﬁed covariates. In this particular case sample sites were selected for spatial balance, which entails that the mean coordinates of sample sites are close to the mean coordinates of all points in the sample frame) and spatial spread (which ensures that the observations are spread out rather than clustered with respect to spatial coordinates), see Grafström and Schelin (2014).
