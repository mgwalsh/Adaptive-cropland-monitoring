...
In this particular case, sample sites were selected purely for spatial balance, which entails that the mean coordinates of sample sites are close to the mean coordinates of all points in the sample frame and have adequate spatial spread. This ensures that the observations are spread out rather than clustered with respect to the spatial coordinates, see [Grafström and Schelin (2014)](https://cran.r-project.org/web/packages/BalancedSampling/index.html). The following chunk performs those calculations:
...
The proposed location coordinates and `GID` can be exported as a GPS readable file (see e.g., [GPSBabel](https://www.gpsbabel.org)), which allows survey crews to navigate to the planned survey locations in the field.
...
Note that the grid ID variable `GID` as well the `Region, District & Ward` administrative units are useful for planning the logistics of the field surveys. However, the `GID` should be the primary spatial georeference point on any given field survey day. Under this particular sampling plan there are ... `GID` in Tanzania. You can also generate a zoomable map of the proposed sampling locations with `leaflet`. This further helps with both navigation and survey planning.
